...
#            ' .android/settings.gradle
            ...
            ' .android/settings.gradle
            #修改相关错误
#            sed -i "" 's/com.android.tools.build:gradle:4.1.0/com.android.tools.build:gradle:7.0.3/g' .android/settings.gradle
#            sed -i "" 's/gradle-6.7-all.zip/gradle-7.0.2-bin.zip/g' .android/gradle/wrapper/gradle-wrapper.properties
        fi
        ...
        if [ `grep -c 'com.github.kezong:fat-aar' .android/build.gradle` -eq '1' ]; then
            ...
            ' .android/build.gradle
             sed -i '' '7i\
             mavenCentral()' .android/build.gradle
#             sed -i '' '18i\
#             mavenCentral()' .android/build.gradle
        fi
        ...
        if [ `grep -c "com.kezong.fat-aar" .android/Flutter/build.gradle` -eq '1' ]; then
            ...
            ' .android/Flutter/build.gradle
            #修改相关错误
            sed -i "" 's/GradleException/FileNotFoundException/g' .android/Flutter/build.gradle
        fi
